...
TEXTURE_PROMPT = """Ты — эксперт по материаловедению и дизайну интерьера.
Проанализируй изображение материала отделки мебели или декора и дай подробное описание его визуальных характеристик на русском языке.

Опиши следующее:
1. **Фактура:** (гладкая, матовая, глянцевая, брашированная, велюр, рогожка, букле, кожа, мрамор с прожилками).
2. **Тип материала:** (дерево, металл, стекло, ткань, камень, пластик).
3. **Цветовая гамма:** (основной цвет, оттенки, паттерн, контрастность).
4. **Тактильное ощущение (визуально):** (теплое/холодное, мягкое/твердое, шершавое/шелковистое).
5. **Стиль:** (современный, лофт, классика, сканди, минимализм, арт-деко).

Верни результат в формате JSON:
{
    "texture_description": "подробное описание фактуры и материала...",
    "material_type": "дерево", // или металл, ткань и т.д.
    "smoothness_score": 8, // 1-10 (1-грубое, 10-глянец)
    "color_analysis": "анализ цвета...",
    "style_tags": ["лофт", "премиум", ...],
    "visual_features": "дополнительные особенности (например, золотые ножки, каретная стяжка)"
}
"""
...
def analyze_textures(max_products: int = None, resume_from: int = 0):
    ...
    with open(catalog_path, "r", encoding="utf-8") as f:
        catalog = [p for p in json.load(f) if p.get('slug')]
