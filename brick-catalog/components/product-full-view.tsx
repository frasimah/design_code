...
import { ArrowLeft, MoreHorizontal, Share, Download, Heart } from "lucide-react";
...
export function ProductFullView({ product, onBack, onSave }: ProductFullViewProps) {
    ...
    const images = [product.main_image, ...(product.gallery || [])].filter((img): img is string => !!img);
        ...
        <div className="max-w-[calc(100%-2rem)] md:max-w-4xl mx-auto w-full py-8 pl-12 md:pl-0">
            ...
            {images.length === 0 ? (
                ...
                <div className="columns-1 sm:columns-2 md:columns-3 gap-4 space-y-4">
                    {images.map((img, idx) => (
                        <div key={idx} className="relative rounded-xl overflow-hidden group bg-secondary/20 break-inside-avoid">
                            <Image
                                src={img}
                            ...
                            />
                            {/* Overlay with actions on hover? For now user just wants grid interface */}
