...
export function SaveToProjectModal({ product, open, onClose, projects, onSave, onCreateProject }: SaveToProjectModalProps) {
    ...
    const imageSrc = product ? ((product.images && product.images.length > 0) ? product.images[0] : product.main_image) : null;
    const title = product ? (product.title || product.name) : "";

    return (
        <Dialog open={open} onOpenChange={onClose}>
            <DialogContent className="sm:max-w-xl bg-[#faf9f5] border-[#1f1e1d0f] rounded-xl shadow-2xl p-0 gap-0 overflow-hidden">
                <DialogHeader className="p-6 pb-2">
                    ...
                    {product && (
                        <div className="flex items-center gap-4 mt-4 bg-white p-2 rounded-lg border border-[#1f1e1d0f]">
                            ...
                            <div>
                                <div className="text-sm font-semibold text-[#141413]">{title}</div>
